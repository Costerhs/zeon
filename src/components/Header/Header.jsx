      ...
      <div className={classNames(hed.boss, location.pathname === '/' && hed.mins)}>
        ...
        <div className={hed.down}>
          ...
          <div className={hed.inp}>
            ...
            <input type="search" value={actual} ref={serk} className={hed.input} placeholder="Поиск" onKeyDown={unkonw} onClick={() => setSearches(true)} onChange={setForResult} />
            <img src={lupa} alt="nice" className={hed.lup} />
            <div className={classNames(hed.over, searches && hed.back)}>

              {cols.map(el => {
                return <div ref={fones} onClick={setForResultHid.bind(this, el)} className={hed.over_item}><p className={hed.p_inp_over}>{el}</p></div>
              })}
            </div>
          </div>
